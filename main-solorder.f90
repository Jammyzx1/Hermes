...
! Version 1.2: Modularization by J. L. McDonagh, September 2016
!
! CHANGE LOG 
! Version 1.1 Addtional error warnings and user output
! Version 1.2 Modularization for incorporation into HERMES.
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
...
subroutine solvent_order(TSname, AtomsBefore, NoWaters)
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
open(unit=4,file='node_feats_cut.txt',status='old',err=100,iostat=ios)			!
read(4,*)NoNodes
NoStrip=NoWaters-NoNodes									!
...
if(ios.gt.0)then
NoNodes=NoWaters									!
print*,'Something wrong with node_feats_cut.txt. Positioning nodes at the origin'		!
open(unit=4,file='node_feats_cut.txt_example',status='unknown')				!
...
open(unit=3, file='node_carts_cut.txt', status='unknown')
write(3,*)NoNodes
write(3,*)'Selected Nodes'
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
   do i=1,NoWaters												!
      ...
      if(configNo.eq.1)  then
         if(i.le.NoNodes) write(3,*)'Ne',NodeCarts(i,1)*b2a,NodeCarts(i,2)*b2a,NodeCarts(i,3)*b2a!output node carts		!
      endif	  
   enddo													!
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!	
   do i=1,NoWaters												!			
...
call system('mv '//trim(TSNAME)//' '//trim(TSNAME)//'_sorted_unstripped')		!Rename new and old training sets so they
call system('mv '//trim(TSNAME)//'_new '//trim(TSNAME))				!are immediately ferebus compatable

end subroutine solvent_order
